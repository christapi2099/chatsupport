...
export default function Home() {
  ...
  const createFirstConversation = useCallback(
    async (userId) => {
  ...
  const createNewConversation = useCallback(
    async (userId, existingConversations) => {
  ...
  const isSportsQuery = (message) => {
    const sportsKeywords = [
      ...
      "match",
      "game",
    ]; // Add more sports-related keywords as needed
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://openrouter.ai/api/v1/chat/completions",
        {
          ...
          body: JSON.stringify({
            messages: [
              ...updatedMessages,
              { role: "system", content: llamaPrompt },
            ],
          }),
        }
      );
      ...
      if (data.choices && data.choices[0] && data.choices[0].message) {
        ...
        const updatedConversationsWithTheme =
          updatedConversationsWithResponse.map((conv) =>
            conv.id === currentConversationId ? { ...conv, title: theme } : conv
          );
  ...
  const calculateFontSize = (title, containerWidth) => {
    ...
    const requiredFontSize =
      (containerWidth / (title.length * estimatedWidthPerChar)) * maxFontSize;
        ...
        >
          <div>
                  ...
                  <ListItemText
                    ...
                    primaryTypographyProps={{
                      style: {
                        ...
                        fontSize: `${calculateFontSize(conv.title, 150)}px`,
                      },
                    }}
                  />
            ...
            <Button
              ...
              fullWidth
              onClick={() =>
                createNewConversation(getAuth().currentUser.uid, conversations)
              }
              sx={{
                marginTop: "10px",
                ...
                >
                  <Box
                    ...
                    sx={{
                      ...
                      borderRadius: "12px",
                      backgroundColor:
                        message.role === "user" ? "#6200ea" : "#03a9f4",
                      color: "white",
                    }}
                  >
                    {message.content}
